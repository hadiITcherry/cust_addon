...
class OdooApiTesting(http.Controller): 
    ...
    def authenticate(self,**kw):
        try:
            ...
            return http.Response(json.dumps({"jsonrpc":"2.0","id":None,"result":{
                ...
                "image":('http://35.180.100.195:8069/web/image/res.partner/%s/image_1920' % user.partner_id.id) if (user.partner_id.image_1920) else ""
        ...
        try:
            ...
            for product in products:
                    product_details.append({
                            ...
                            "image":'http://35.180.100.195:8069/web/image/product.template/%s/image_1920' % product.id
                        })
            pagination = {
        ...
        try:
            ...
            for product in products:
                ...
                product_details={
                    ...
                    'images':list(map(lambda img:('http://35.180.100.195:8069/web/image/product.image/%s/image_1920' % img.id), images))
                }
        ...
        try:
            ...
            for product in products:
                product_details.append({
                    ...
                    'new_price':product.vip_price if product.vip == True else product.sale_price if product.sale == True else 0.0,
                    'category':product.categ_id.name,
                    "image":('http://35.180.100.195:8069/web/image/product.image/%s/image_1920' % product.id) if product.image_1920 != False else ""
                })
            pagination = {
                    "page":page_number if page_number > 0 else 1,
                    "size":0 if product_details == [] else len(product_details),
            ...
            for cat in category:
                if cat.parent_id.id == False:
                        categories.append({
                            ...
                            "image":('http://35.180.100.195:8069/web/image/product.category/%s/image' % cat.id) if cat.image != False else ""
                        })
        ...
        try:
            ...
            for product in products:
                products_list.append({
                    ...
                    'new_price':product.vip_price if product.vip == True else product.sale_price if product.sale == True else 0.0,
                    'category':product.categ_id.name,
                    'image':('http://35.180.100.195:8069/web/image/product.template/%s/image_1920' % product.id) if product.image_1920 != False else ""
                })
            pagination = {
                    "page":page_number if page_number > 0 else 1,
            ...
            for brand in brand:
                ...
                for b in brand_values:
                    brands.append({
                        "id":b.id,
                        "name":b.name,
                        "image":('http://35.180.100.195:8069/web/image/product.attribute.value/%s/image' % b.id) if b.image != False else ""
                    })
                ...
                for cart in cart_list:
                    ...
                    for p in product:
                        ...
                        cart_data.append({
                                ...
                                "in_stock":status,
                                "image":('http://35.180.100.195:8069/web/image/product.template/%s/image_1920' % p.id)  if p.image_1920 != False else ""
                            })
        ...
        try:
            ...
            for serv in service_type:
                service_types.append({
                    ...
                    "image":('http://35.180.100.195:8069/web/image/fsm.order.type/%s/image' % serv.id) if serv.image != False else "",
        ...
        try:
            ...
            for serv in service:
                services.append({
                    ...
                    "image":('http://35.180.100.195:8069/web/image/fsm.tag/%s/icon' % serv.id) if serv.icon != False else ""
                })
        ...
        try:
            if(http.request.session.uid != None):
                ...
                if service:
                    for order in service:
                        if (order.image_1):
                            images.append('http://35.180.100.195:8069/web/image/fsm.order/%s/image_1' % order.id)
                        if (order.image_2):
                            images.append('http://35.180.100.195:8069/web/image/fsm.order/%s/image_2' % order.id)
                        if (order.image_3):
                            images.append('http://35.180.100.195:8069/web/image/fsm.order/%s/image_3' % order.id)
                        if (order.image_4):
                            images.append('http://35.180.100.195:8069/web/image/fsm.order/%s/image_4' % order.id)
                        service_order = {
                                'id':order.id,
                                'service_nb':order.name,
            ...
            if(http.request.session.uid != None):
                ...
                for s in service:
                    	encoded_body = {
			                ...
			                'images':[('http://35.180.100.195:8069:8069/web/image/fsm.order/%s/c_images' % s.id) if s.c_images != False else ""]
                        }
    ...
    def home_page(self):
        ...
        try:
            ...
            if banner:
                if banner.image_1:
                    banners.append('http://35.180.100.195:8069/web/image/banners/%s/image_1' % banner.id)
                if banner.image_2:
                    banners.append('http://35.180.100.195:8069/web/image/banners/%s/image_2' % banner.id)
                if banner.image_3:
                    banners.append('http://35.180.100.195:8069/web/image/banners/%s/image_3' % banner.id)
                if banner.image_4:
                    banners.append('http://35.180.100.195:8069/web/image/banners/%s/image_4' % banner.id)
                if banner.image_5:
                    banners.append('http://35.180.100.195:8069/web/image/banners/%s/image_5' % banner.id)
            service = http.request.env['fsm.tag'].sudo().search([])
            services= []
            for serv in service:
                services.append({
            ...
            for v in vip:
                vip_products.append({
                    ...
                    "image":('http://35.180.100.195:8069/web/image/product.template/%s/image_1920' % v.id) if (v.image_1920) else ""
                }) 
            brand = http.request.env['product.attribute'].sudo().search([('name','=',"BRAND")])
            brands = []
            for brand in brand:
            ...
            for h in hot_deal:
                hot_deals.append({
                    ...
                    "image":('http://35.180.100.195:8069/web/image/product.template/%s/image_1920' % h.id) if (h.image_1920) else ""
                })
            ...
            for o in offer:
                offers.append({
                    ...
                    "image":('http://35.180.100.195:8069/web/image/product.template/%s/image_1920' % o.id) if (o.image_1920) else ""
                }) 
                return http.Response(json.dumps({"jsonrpc":"2.0","id":None,"result":{"banners":banners,"services":services,"categories":categories,"vip_products":vip_products,"brands":brands,"bundles":bundles,"challenges":challenges,"hot_deals":hot_deals,"best_selling":sorted_products,"offers":offers}}),headers=headers)
        except Exception as e:
            return http.Response(json.dumps({"jsonrpc":"2.0","id":None,"error":str(e)}),headers=headers)
